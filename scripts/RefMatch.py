...
def main(args):
	...
	for (distance, i, j) in allpairs:
		...
		ifdup = 0
		if thepair in finish_haplos:
			ifdup = 1
		
		best_matches[headers_list[i]].append( (distance, haplo2 , ifdup  ) )
	
		if not ifdup:
			finish_haplos.add(thepair)
	...
	for (distance, i, j) in allpairs:
		...
		best_matches[headers_list[i]].append( (distance, headers_list[j], 1) )
	...
	with open(outputfile, mode = 'w') as f:
		...
		for header in headers:
			...
			if header in best_matches:
				...
				for (distance, ref, iffirst) in best_matches[header][::-1]:
